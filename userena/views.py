...
class ProfileListView(ListView):
    ...
    def get_queryset(self):
        ...
        ganaderia = Ganaderia.objects.get(perfil=self.request.user)
        queryset = profile_model.objects.get_visible_profiles(self.request.user).select_related().filter(ganaderia_perfil=ganaderia)
...
from profiles.models import Ganaderia
@secure_required
def add_profile(request, signup_form=SignupForm,
           template_name='userena/add_profile.html', success_url=None,
           extra_context=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ganaderia = Ganaderia.objects.get(perfil=user_active)
            ganaderia.perfil.add(user.id)
            ...
            return redirect('userena_profile_list')
...
def signout(request, next_page=userena_settings.USERENA_REDIRECT_ON_SIGNOUT,
            template_name='home.html', *args, **kwargs):
    """
